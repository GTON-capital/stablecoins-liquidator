...
export async function tryFetchPrice(token: string, amount: bigint, decimals: number): Promise<string> {
  ...
  try {
    if (keydonixOracleTypes.includes(oracleType)) {
      return (
        '$' + formatNumber(await tryFetchKeydonixPrice(token, amount, decimals, oracleType, oracleAddress, blockNumber))
      )
    }
...
export async function tryFetchKeydonixPrice(
  token: string,
  amount: bigint,
  decimals: number,
  oracleType: number,
...
export function encodeLiquidationTriggerWithProof(
  asset: string,
  owner: string,
...
export async function getPriceFromOracle(
  oracle: string,
  token: string,
  decimals: number,
  amount: bigint,
...
export async function getPriceFromOracleKeydonix(
  oracle: string,
  token: string,
  proofData,
  decimals: number,
  ...
  switch (oracleType) {
    ...
    case ORACLE_TYPES.KEYDONIX_SHIBA:
      return getMerkleProof(
        BigInt(getLPAddressByOracle(oracleType, token, WETH)),
        denominationToken,
        BigInt(proofBlockNumber),
      )
    default:
...
export async function getProofForWrapped(
  token: string,
  blockNumber: number,
): Promise<[string, string, string, string]> {
...
export async function getTokenSymbol(token: string) {
  ...
  try {
    if (['UNI-V2', 'SLP'].includes(symbol)) {
      const token0 = web3.eth.abi.decodeParameter(
        'address',
        await web3.eth.call({
          to: token,
          data: '0x0dfe1681',
      ...
      const token1 = web3.eth.abi.decodeParameter(
        'address',
        await web3.eth.call({
          to: token,
          data: '0xd21220a7',
...
export async function getAllCdps(blockNumber: number): Promise<{ asset: string; owner: string }[]> {
  ...
  return web3.eth.abi
    ...
    .map((x) => ({ owner: x['owner'], asset: x['asset'] }))
}
...
export async function getAllCdpsData(blockNumber: number): Promise<Map<string, CDP>> {
  ...
  for (const [idx, asset] of assets.entries()) assetToOracleTypeMap[asset] = oracles[idx]
  ...
  for (const cdp of positions) result.set(`${cdp.asset}_${cdp.owner}`, cdp)
